...
class DateTimeComponent extends React.Component
{
	...
	return (
	   ...
	   le {this.state.datetime }
	    <br/>
	    <input value={this.state.date} onChange={ (ev) => {this.setState({ date: ev.target.value}); this.processChange()} } type='date'/>
	    <input value={this.state.time}  type='time' onMouseLeave={ ()=>this.processChange() }
	    onChange={ (ev) => {this.setState({ time: ev.target.value}); this.processChange()} }
	    />
	    </div>)
    }
}
